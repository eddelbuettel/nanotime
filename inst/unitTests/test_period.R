...
test_as.period_character <- function() {
    p1 <- as.period("1m1d")
    checkIdentical(period.day(p1), 1)
    checkIdentical(period.month(p1), 1)
    checkIdentical(period.duration(p1), as.duration(0))

    checkIdentical(as.period("1y"), as.period("12m"))
    checkIdentical(as.period("2y"), as.period("24m"))
    checkIdentical(as.period("1w"), as.period("7d"))
    checkIdentical(as.period("2w"), as.period("14d"))
    
    p2 <- as.period("2m1d/00:01:01.1")
    checkIdentical(period.month(p2), 2)
    checkIdentical(period.day(p2), 1)
    checkIdentical(period.duration(p2), as.duration("00:01:01.1"))

    p3 <- as.period(c("1d","2d","3d"))
    checkIdentical(period.day(p3[1]), 1)
    checkIdentical(period.day(p3[2]), 2)
    checkIdentical(period.day(p3[3]), 3)    
    ...
    checkIdentical(names(as.period(c(a="1m"))), "a")
}
...
test_as.period_integer64 <- function() {
    ...
    checkIdentical(period.duration(p1), as.duration(1:10))
    p1 <- as.period(c(a=as.integer64(1)))
    checkIdentical(period.duration(p1), as.duration(c(a=1)))
}
...
test_as.period_numeric <- function() {
    ...
    checkIdentical(period.duration(p1), as.duration(1:10))
    p1 <- as.period(c(a=1))
    checkIdentical(period.duration(p1), c(a=as.duration(1)))
}
test_as.period_duration <- function() {
    p1 <- as.period(c(a=as.duration(1)))
    checkIdentical(p1, c(a=as.period("00:00:00.000_000_001")))
}
test_period <- function() {
    checkIdentical(period(0,0,1), as.period(1))
    checkIdentical(period(0,0,1:10), as.period(1:10))
    checkIdentical(period(1,1,0:9), as.period(paste0("1m1d/00:00:00.000_000_00", 0:9)))
}
...
test_subset_int <- function() {
    ...
    pp <- c(p1, p2, p3, p4)
    checkIdentical(pp[1], p1)
    checkIdentical(pp[2], p2)
    checkIdentical(pp[3], p3)
    checkIdentical(pp[4], p4)
    checkIdentical(pp[1:4], pp)
    checkIdentical(pp[2:3], c(p2,p3))
}
test_subset_logical <- function() {
    ...
    p4 <- as.period("4m4d/00:00:04") 
    pp <- c(p1, p2, p3, p4)
    checkIdentical(pp[c(T,F,F,F)], p1)
    checkIdentical(pp[c(F,T,F,F)], p2)
    checkIdentical(pp[c(F,F,T,F)], p3)
    checkIdentical(pp[c(F,F,F,T)], p4)
    checkIdentical(pp[TRUE], pp)
    checkIdentical(pp[c(F,T,T,F)], c(p2,p3))
}
test_subset_character <- function() {
    pp <- c(x=as.period(1), y=as.period(2))
    checkIdentical(pp["x"], c(x=as.period(1)))
    checkIdentical(pp["y"], c(y=as.period(2)))
    ## checkIdentical(pp["a"], as.period(as.integer64(NA)))    LLL
}
...
test_subsassign_character <- function() {
    ...
    checkIdentical(pp, expected)
}
test_square_bracket <- function() {
    pp <- c(a=as.period(1), b=as.period(2), c=as.period(3), d=as.period(4))
    pp_nonames <- as.period(1:4)
    checkIdentical(pp_nonames[1], pp[[1]])
    checkIdentical(pp_nonames[2], pp[[2]])
    checkIdentical(pp_nonames[3], pp[[3]])
}

## ops
## -
test_period_minus_period <- function() {
    checkIdentical(as.period("2m2d") - as.period("1m1d"), as.period("1m1d"))
    checkIdentical(as.period("-1m-1d/-00:00:01") - as.period("1m1d/00:00:01"),
                as.period("-2m-2d/-00:00:02"))
}
test_period_minus_period_vector <- function() {
    checkIdentical(as.period("1d") - as.period(paste0(1:10, "d")),
                as.period(paste0(0:-9,"d")))
    checkIdentical(as.period(paste0(1:10, "d")) - as.period("1d"),
                as.period(paste0(0:9,"d")))
    checkIdentical(as.period(paste0(1:10, "d")) - as.period(paste0(0:9, "d")),
                as.period(paste0(rep(1,10),"d")))
}
test_period_minus_numeric <- function() {
    checkIdentical(as.period("2m2d") - 1,
                as.period("2m2d/-00:00:00.000_000_001"))
    checkIdentical(as.period(paste0(1:10,"m2d")) - 1,
                as.period(paste0(1:10, "m2d/-00:00:00.000_000_001")))
    checkIdentical(as.period("12m2d") - 1:9.0,
...
test_period_minus_duration <- function() {
    ...
    checkIdentical(as.period(paste0(1:10,"m2d")) - as.duration(1),
                as.period(paste0(1:10, "m2d/-00:00:00.000_000_001")))
    checkIdentical(as.period("12m2d") - as.duration(1:9),
                as.period(paste0("12m2d/-00:00:00.000_000_00", 1:9)))
}
test_period_minus_integer <- function() {
    checkIdentical(as.period("2m2d") - as.integer(1), as.period("2m2d/-00:00:00.000_000_001"))
    checkIdentical(as.period(paste0(1:10,"m2d")) - as.integer(1),
                as.period(paste0(1:10, "m2d/-00:00:00.000_000_001")))
    checkIdentical(as.period("12m2d") - as.integer(1:9),
                as.period(paste0("12m2d/-00:00:00.000_000_00", 1:9)))
}
test_period_minus_integer64 <- function() {
    checkIdentical(as.period("2m2d") - as.integer64(1), as.period("2m2d/-00:00:00.000_000_001"))
    checkIdentical(as.period(paste0(1:10,"m2d")) - as.integer64(1),
                as.period(paste0(1:10, "m2d/-00:00:00.000_000_001")))
    checkIdentical(as.period("12m2d") - as.integer64(1:9),
                as.period(paste0("12m2d/-00:00:00.000_000_00", 1:9)))
}
test_numeric_minus_period <- function() {
    checkIdentical(1 - as.period("1m1d"), as.period("-1m-1d/00:00:00.000_000_001"))
    checkIdentical(1:10 - as.period("1m1d"), period(-1,-1,1:10))
    checkIdentical(1 - period(1:10,1,1), period(-1:-10, -1, 0))
}
test_integer64_minus_period <- function() {
    checkIdentical(as.integer64(1) - as.period("1m1d"),
                as.period("-1m-1d/00:00:00.000_000_001"))
    checkIdentical(as.integer64(1:10) - as.period("1m1d"), period(-1,-1,1:10))
    checkIdentical(as.integer64(1) - period(1:10,1,1), period(-1:-10, -1, 0))
}
test_duration_minus_period <- function() {
    checkIdentical(as.duration(1) - as.period("1m1d"),
                as.period("-1m-1d/00:00:00.000_000_001"))
    checkIdentical(as.duration(1:10) - as.period("1m1d"), period(-1,-1,1:10))
    checkIdentical(as.duration(1) - period(1:10,1,1), period(-1:-10, -1, 0))
}
...
test_period_plus_period <- function() {
    checkIdentical(as.period("2m2d") + as.period("1m1d"), as.period("3m3d"))
    checkIdentical(as.period("-1m-1d/00:00:01") + as.period("1m1d/00:00:01"),
                   as.period("0m0d/00:00:02"))
    checkIdentical(period(1,1,1:10) + period(1,1,0), period(2,2,1:10))
    checkIdentical(period(1,1,0) + period(1,1,1:10), period(2,2,1:10))
}
test_period_plus_period_names <- function() {
  ...
  checkIdentical(non_scalar + scalar, expected)
  checkIdentical(scalar + non_scalar, expected)
}
test_integer64_plus_period <- function() {
    checkIdentical(as.integer64(1) + as.period(1), as.period(2))
    checkIdentical(as.integer64(0:9) + as.period(1), as.period(1:10))
    checkIdentical(as.integer64(1) + period(1:10,1,0), period(1:10,1,1))
}
test_period_plus_integer64 <- function() {
    checkIdentical(as.period(1) + as.integer64(1), as.period(2))
    checkIdentical(as.period("2m2d") + as.integer64(1), as.period("2m2d/00:00:00.000_000_001"))
    checkIdentical(as.period(1) + as.integer64(0:9), as.period(1:10))
    checkIdentical(period(1:10,1,0) + as.integer64(1), period(1:10,1,1))
}
test_period_plus_duration <- function() {
    checkIdentical(as.period(1) + as.duration(1), as.period(2))
    checkIdentical(as.period("2m2d") + as.duration(1), as.period("2m2d/00:00:00.000_000_001"))
    checkIdentical(as.period(1) + as.duration(0:9), as.period(1:10))
    checkIdentical(period(1:10,1,0) + as.duration(1), period(1:10,1,1))
}
test_duration_plus_period <- function() {
    checkIdentical(as.duration(1) + as.period(1), as.period(2))
    checkIdentical(as.duration(1) + as.period("2m2d"), as.period("2m2d/00:00:00.000_000_001"))
    checkIdentical(as.duration(0:9) + as.period(1), as.period(1:10))
    checkIdentical(as.duration(1) + period(1:10,1,0), period(1:10,1,1))
}
test_numeric_plus_period <- function() {
    checkIdentical(as.period(1) + 1, as.period(2))
    checkIdentical(as.period("2m2d") + 1, as.period("2m2d/00:00:00.000_000_001"))
    checkIdentical(0.0:9.0 + as.period(1), as.period(1:10))
    checkIdentical(1.0 + period(1:10,1,0), period(1:10,1,1))
}
...
test_period_times_numeric <- function() {
    checkIdentical(as.period(1) * 3, as.period(3))
    checkIdentical(as.period("1m1d") * 3, as.period("3m3d"))
    checkIdentical(period(1,1,1) * 1:10, period(1:10,1:10,1:10))
    checkIdentical(period(1:10,1,1) * 3, period(1:10 * 3, 3, 3))
}
test_period_times_integer64 <- function() {
    checkIdentical(as.period(1) * as.integer64(3), as.period(3))
    checkIdentical(period(1,1,1) * as.integer64(1:10), period(1:10,1:10,1:10))
    checkIdentical(period(1:10,1,1) * as.integer64(3), period(1:10 * 3, 3, 3))
}
test_numeric_times_period <- function() {
    checkIdentical(3 * as.period(1), as.period(3))
    checkIdentical(4.5 * as.period("10d"), as.period("45d"))
    checkIdentical(1:10.0 * as.period(1), as.period(1:10))
    checkIdentical(1 * as.period(1:10), as.period(1:10))
}
test_integer64_times_period <- function() {
    checkIdentical(as.integer64(3) * as.period(1), as.period(3))
    checkIdentical(as.integer64(3) * as.period("1m1d"), as.period("3m3d"))
    checkIdentical(as.integer64(1:10) * as.period(1), as.period(1:10))
    checkIdentical(as.integer64(1) * as.period(1:10), as.period(1:10))
}
...
test_period_div_numeric <- function() {
    checkIdentical(as.period(4) / 3, as.period(1))
    checkIdentical(as.period("5m5d") / 2.5, as.period("2m2d"))
    checkIdentical(as.period(4) / c(4,2,1), as.period(c(1,2,4)))
    checkIdentical(as.period(4:2) / c(4,2,1), as.period(c(1,1,2)))
    checkException(as.period("2m") / 0, "divide by zero")
}
test_period_div_integer64 <- function() {
    checkIdentical(as.period(4) / as.integer64(3), as.period(1))
    checkIdentical(as.period("5m5d") / as.integer64(2), as.period("2m2d"))
    checkIdentical(as.period(4) / as.integer64(c(4,2,1)), as.period(c(1,2,4)))
    checkIdentical(as.period(4:2) / as.integer64(c(4,2,1)), as.period(c(1,1,2)))
}
test_period_div_integer <- function() {
    checkIdentical(as.period(4) / as.integer(3), as.period(1))
    checkIdentical(as.period("5m5d") / as.integer(2), as.period("2m2d"))
    checkIdentical(as.period(4) / as.integer(c(4,2,1)), as.period(c(1,2,4)))
    checkIdentical(as.period(4:2) / as.integer(c(4,2,1)), as.period(c(1,1,2)))
}
...
test_period_eq_period <- function() {
    ...
    checkIdentical(as.period(1:10) == as.period(1:10), rep(TRUE, 10))
}
test_period_ne_period <- function() {
    checkTrue(as.period(1) != as.period(2))
    checkTrue(as.period("1d") != as.period("2d"))
    checkTrue(!(as.period(1) != as.period(1)))
    checkIdentical(as.period(1:10) != as.period(1:10), rep(FALSE, 10))
}
...
test_period_get_names <- function() {
    a <- as.period(1:10)
    names(a) <- "b"
    checkIdentical(names(a), c("b", rep(as.character(NA), 9)))
}    
test_period_set_names <- function() {
    names <- c("a","b","c","d")
    pp <- as.period(1:4)
    names(pp) <- names
    checkIdentical(names(pp), names)
    names(pp)[1] <- "x"
    checkIdentical(names(pp), c("x","b","c","d"))
}    

test_period_c <- function() {
    pp <- c(x=as.period(1), y=as.period(2))
    checkIdentical(names(pp), c("x","y"))
    checkIdentical(pp[1], c(x=as.period(1)))
    checkIdentical(pp[2], c(y=as.period(2)))

    pp <- c(as.period(1:10), as.period(11:20))
    checkIdentical(pp, as.period(1:20))
}
