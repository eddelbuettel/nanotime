...
test_nanotime_data_frame_constructor <- function() {
  ...
  checkIdentical(newdf$rain,  rain)
  checkIdentical(newdf$shine, shine)
}
test_nanotime_data_frame_rbind <- function() {
  ...
  df <- rbind(df1, df2)
  checkIdentical(df, data.frame(t = c(t1, t2), d = c(d1, d2)))
}
test_nanotime_data_frame_cbind <- function() {
  ...
  df <- cbind(df1, df2)
  checkIdentical(df, data.frame(t1 = t1, d1 = d1, t2=t2, d2=d2))
}
