...
abstract: |
  ...
  The \pkg{nanotime} package \citep{nanotime2020} provides a coherent set of
  ...
  both a finer resolution and additional operations.
...
based on calendar time as a year or a day may have different length depending on its location in time
and depending in which time zone it is considered.
<!-- DE: do we need an example of that? -->
...
All the new types introduce by the \pkg{nantime} package are compatible with `data.frame` and
`data.table` objects. By having an ordered `nanotime` column it is thus easy to define a
time-series. One can then use `nanoival` subsetting.
...
Use of the rounding functions make it is possible to perform aggregations on `data.table` instances
...
```
<!--Nope:  as.nanoperiod("12m1s/01:00:00") -->
